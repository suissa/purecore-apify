...
usersRouter.post('/', (req: Request, res: Response) => {
  ...
  if (!userData.name || typeof userData.name !== 'string') {
    throw validationError('Nome é obrigatório e deve ser uma string', 'name', userData.name);
  }

  if (!userData.email || typeof userData.email !== 'string') {
    throw validationError('Email é obrigatório e deve ser uma string', 'email', userData.email);
  }

  // Validação de email simples
  const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
  if (!emailRegex.test(userData.email)) {
    throw validationError('Email deve ter formato válido', 'email', userData.email);
  }
