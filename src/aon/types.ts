...
export interface AONStreamWriter {
  ...
  healing(action: string, description: string, severity?: AONSeverity, metadata?: Record<string, any>): void;
}

export interface AONHealer {
  /** Executa uma ação de healing */
  heal(action: string, description: string, metadata?: Record<string, any>): Promise<boolean>;
...
export function createAONEvent<T extends AONEvent>(
  type: T['type'],
