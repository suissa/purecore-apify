...
import { Logs, Metrics, TraceSpan } from './observability';
import { CircuitBreaker, Timeout, Failover } from './resilience';
import { AuthExpressGuard, XSSGuard as XSSGuardSecurity, CSRFGuard as CSRFGuardSecurity, AuthJWTGuard, IdempotentGuard as IdempotentGuardSecurity } from './security';
import { CQRS as CQRSPerformance } from './performance';

// Imports dos novos decorators
import { PresetDecoratorFactory } from './preset';
import { SchemaValidator, ZodValidator, JoiValidator, YupValidator, AjvValidator } from './schema-validator';
import { Memoization, SmartCache, ApiCache } from './memoization';
import { Inject, LazyInject, InjectMethod, registerDependency, resolveDependency } from './injection';
import { Catch, CatchHttpErrors, CatchValidationErrors, CatchWithRetry } from './catch';
...
import { Request, Response, NextFunction, RequestHandler } from '../../src/types';
...
// Tipo compatível para decorators
export type DecoratorFunction = (target: any, propertyKey: string | symbol | undefined, descriptor?: PropertyDescriptor) => any;
...
/**
 * Wrapper para SmartCache que torna compatível com DecoratorFunction
 */
function createSmartCacheWrapper(options: any): DecoratorFunction {
  return (target: any, propertyKey: string | symbol | undefined, descriptor?: PropertyDescriptor) => {
    if (typeof propertyKey === 'string' && descriptor) {
      return SmartCache(options)(target, propertyKey, descriptor);
    }
    return descriptor;
  };
}
...
/**
 * Wrapper para ApiCache que torna compatível com DecoratorFunction
 */
function createApiCacheWrapper(ttl: number): DecoratorFunction {
  return (target: any, propertyKey: string | symbol | undefined, descriptor?: PropertyDescriptor) => {
    if (typeof propertyKey === 'string' && descriptor) {
      return ApiCache(ttl)(target, propertyKey, descriptor);
    }
    return descriptor;
  };
}
...
/**
 * Wrapper para Memoization que torna compatível com DecoratorFunction
 */
function createMemoizationWrapper(options: any): DecoratorFunction {
  return (target: any, propertyKey: string | symbol | undefined, descriptor?: PropertyDescriptor) => {
    if (typeof propertyKey === 'string' && descriptor) {
      return Memoization(options)(target, propertyKey, descriptor);
    }
    return descriptor;
  };
}
...
/**
 * Wrapper para CatchHttpErrors que torna compatível com DecoratorFunction
 */
function createCatchHttpErrorsWrapper(options: any): DecoratorFunction {
  return (target: any, propertyKey: string | symbol | undefined, descriptor?: PropertyDescriptor) => {
    if (typeof propertyKey === 'string' && descriptor) {
      return CatchHttpErrors(options)(target, propertyKey, descriptor);
    }
    return descriptor;
  };
}
...
/**
 * Wrapper para CatchWithRetry que torna compatível com DecoratorFunction
 */
function createCatchWithRetryWrapper(retries: number, delay: number): DecoratorFunction {
  return (target: any, propertyKey: string | symbol | undefined, descriptor?: PropertyDescriptor) => {
    if (typeof propertyKey === 'string' && descriptor) {
      return CatchWithRetry(retries, delay)(target, propertyKey, descriptor);
    }
    return descriptor;
  };
}
...
/**
 * Wrapper genérico para MethodDecorator
 */
function createMethodDecoratorWrapper(decorator: MethodDecorator): DecoratorFunction {
  return (target: any, propertyKey: string | symbol | undefined, descriptor?: PropertyDescriptor) => {
...
export const AutoescaleSentinel = PresetDecoratorFactory([
  ...
  Metrics,
  TraceSpan,
  createSmartCacheWrapper({ ttl: 300 }),
  createMethodDecoratorWrapper(CircuitBreaker({ failureThreshold: 5, resetTimeoutMs: 10000 })),
  createMethodDecoratorWrapper(Timeout({
    ms: 30000,
    maxTimeoutMs: 60000,
    retryAttempts: 3,
...
export const ApifyCompleteSentinel = PresetDecoratorFactory([
  ...
  createMethodDecoratorWrapper(AuthJwtGuard({ secret: process.env.JWT_SECRET })),
  XSSGuard,

  // Helmet - Headers de segurança HTTP completos
  createMethodDecoratorWrapper(HelmetGuard({
...
export const ApiSentinel = PresetDecoratorFactory([
  Logs,
  Metrics,
  createApiCacheWrapper(300), // 5 minutos de cache
  createCatchHttpErrorsWrapper({ logError: true }),
  AuthExpressGuard
]);
...
export const DatabaseSentinel = PresetDecoratorFactory([
  ...
  createMemoizationWrapper({ ttl: 60 }), // Cache por 1 minuto
  createCatchWithRetryWrapper(3, 1000) // 3 tentativas com 1s de delay
]);
...
export const ExternalApiSentinel = PresetDecoratorFactory([
  ...
  createMemoizationWrapper({ ttl: 300 }), // Cache por 5 minutos
  createCatchWithRetryWrapper(2, 2000) // 2 tentativas com 2s de delay
]);
